...
def smooth(image, sigma=1.4, length=5):
    ...
    for i in range(W - 2 * k):
        for j in range(H - 2 * k):
            ...
            t_tec=image[i:i + length, j:j + length]
            t_data=image[i:i + length, j:j + length] * gaussian
...
if __name__ == "__main__":
    # code to read image
    image = cv.imread('data/OIP-C.jpg', 0)
