...
list1 = np.genfromtxt(open("../exportdata/145.csv", "rb"), delimiter=",", encoding='GBK')
sorted_data=[]
sorted_datay=[]
x5,x1=[],[]
...
for i in range(count[4.0]):
    for j in range(count[3.0]):
        if ((Value[no4 + i] - Value[count[no3]+j]) > 9.6) and ((Value[no4 + i] - Value[count[no3]+j]) < 9.8):
...
for i in range(count[5.0]):
    for j in range(count[4.0]):
        if ((Value[no5 + i] - Value[j]) > 149.68) and ((Value[no5 + i] - Value[j]) < 149.8):
            D.append(Value[no5 + i] - Value[j])
            D_i.append(i)
            D_j.append(j)
...
plt.figure()
plt.plot(D_j)
np.savetxt("5.txt", C1, fmt='%f', delimiter=',')  # 数组保存
plt.show()
