...
# valid = False
    list1 = np.genfromtxt(open("../exportdata/16-111.txt", "rb"), delimiter=",", encoding='GBK')
    for i in range(len(list1)):
        if list1[i][2] == 1:
            vaild, lenth_tem = findnearpointA.findpoint(True, False, list1[i][3] * 0.025, 0, 700, 9011)
        elif list1[i][2] == 2:
            vaild, lenth_tem = findnearpointA.findpoint(False, True, 0, list1[i][3] * 0.025, 700, 9011)
            findnearpointB.findpoint(True, False, list1[i][3] * 0.025, 0, 700, 59108)
            if vaild:
                lenthA.append(lenth_tem)
        elif list1[i][2] == 3:
            vaild, lenth_tem = findnearpointB.findpoint(False, True, 0, list1[i][3] * 0.025, 700, 59108)
            findnearpointC.findpoint(True, False, list1[i][3] * 0.025, 0, 700, 12778)
            if vaild:
                lenthB.append(lenth_tem)
        elif list1[i][2] == 4:
            vaild, lenth_tem = findnearpointC.findpoint(False, True, 0, list1[i][3] * 0.025, 700, 12778)
            findnearpointD.findpoint(True, False, list1[i][3] * 0.025, 0, 700, 69441)
            if vaild:
                lenthC.append(lenth_tem)
        elif list1[i][2] == 5:
            vaild, lenth_tem = findnearpointD.findpoint(False, True, 0, list1[i][3] * 0.025, 1500, 69441)
            if vaild:
