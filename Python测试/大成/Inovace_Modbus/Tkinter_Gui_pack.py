...
class DI_Lable():
    def __init__(self,DIn,root,DI_DO_Status=False):
        ...
        fm = [Frame(root)]*16
        if DI_DO_Status:
            Label(fm[DIn], text='第 {0} 组D0信号'.format(DIn) ,height =3,width=10, justify='right').pack(side='left',padx=3,pady=3)
        else:
            Label(fm[DIn], text='第 {0} 组DI信号'.format(DIn) , height=3,width=10, justify='right').pack(side='left',padx=3,pady=3)
        for i in range(16):
            self.DI[i] = Label(fm[DIn], text="DI{0}".format(i), background='gray', foreground='blue',height=1, width=3, justify='left')
            self.DI[i].pack(side='left',padx=3,pady=3)
        fm[DIn].pack(side='top')
...
root=Tk()
root.resizable(0,0)
root.title('Inovance_PLC_IO_Read_V2.1')
...
def datachange():
    global Machin_num
    tup_data= Creat_Com.read_data(addr=4899+11*Machin_num,data_len=11)
...
D.pack(side='left',padx=20, pady=20)
