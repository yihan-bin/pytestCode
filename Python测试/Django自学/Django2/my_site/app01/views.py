...
def book6(request,year,name,version):
    return HttpResponse('article_字写url转换器'+name)
def sql_test(request):
    con=pymysql.connect(host='127.0.0.1',port=3306,user='root',passwd='admin',db='luffy_db')
    cursor=con.cursor()
    cursor.execute('select  username,password from web_account;')
